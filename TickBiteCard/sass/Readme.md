...
folder are mapped to the application's root namespace, 'TickBiteCard'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "TickBiteCard"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'TickBiteCard' in the same way
as `"TickBiteCard/sass/src"`.
